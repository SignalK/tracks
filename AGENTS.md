...
- **Tracks carry a human name.** `Own Ship` or `AIS <shipname>`, falling back to the MMSI and then the raw context — the convention TimeZero's GPX exports use, and what makes a list of tracks readable. `trackName()` in `utils.ts` resolves it per request, because an AIS static report can arrive long after the first position. Note a vessel's `name` is a **bare string** in the full data model, not the `{value}` wrapper `navigation.position` carries; both are accepted. `app.getPath` is optional — older servers fall back to the MMSI.
