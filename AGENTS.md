...
- **Coordinate order flips at the boundary.** Internally positions are `[lat, lng]` (`LatLngTuple`); GeoJSON output is `[lng, lat]` via `toLngLat`. The `bbox` query parameter is `lat,lon,lat,lon`, matching the internal order rather than the GeoJSON one. Check which side of that boundary you're on before "fixing" an order that looks wrong.
