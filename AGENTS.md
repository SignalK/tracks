...
- **GPX parsing goes through a real XML parser.** `gpx.ts` reads with `@xmldom/xmldom` and writes by hand. It was regex all the way through until [#91](https://github.com/SignalK/tracks/pull/91), and the reason it changed is worth keeping: a pattern cannot tell markup from text, so a commented-out `<trk>` became a real track and a `<trkseg>` inside CDATA injected points the file never declared. Those are not edge cases a pattern can be extended to cover. Two consequences follow. **A document that is not well-formed yields no tracks** — an undeclared namespace prefix, a mismatched closing tag — rather than the readable half, because returning part of a broken file claims a success it does not support. And **an invalid character reference does not survive a round trip**: the parser decodes `&#0;` or `&#xD800;` into a character XML cannot express, and `toGpx` drops it on the way out. Values are still checked hard on top of the parser: a coordinate must be an `xsd:decimal` within ±90/±180, a `<time>` must be an `xsd:dateTime` (`Date.parse` alone reads a bare `2024` and rolls `2024-02-30` into March), and the Signal K context is matched on its **namespace URI** in the track's own `<extensions>` — never a trackpoint's, and never by local name, or a foreign `<context>` would claim the vessel's identity. **Points are sorted by time within each segment** and deliberately not across them: `src/__fixtures__/timezero-seams.gpx` reproduces TimeZero's overlapping 150-point block seams, and the real vendor exports run when `TZ_EXPORT_DIR` points at them and skip when it does not. Coordinates are written at seven decimal places (~11 mm), so a round trip moves a position by at most the 5.6 mm half-ULP bound.
