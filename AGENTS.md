...
- **Both single-track routes read through `readSegments`.** The JSON route and `track.gpx` share one pipeline, so the same query cannot be answered two ways: the history reconciliation, the windowless fallback that still consults a provider, and the rule that a 404 means _neither source knows this vessel_ are subtle enough that a second copy drifts. Export is a route rather than client code for the same reason — `toGpx` stays the single serialiser. **Honouring that 404 rule takes a second provider call**: `getValues` narrowed to a window cannot tell an unknown vessel from one whose history lies outside it, so when a track is about to 404 the provider's `getContexts` is asked whether it knows the vessel at all. Best-effort like every other provider call — a provider that lacks the method, errors or hangs leaves the 404 standing — and it costs nothing on the common path, which never reaches that branch.
