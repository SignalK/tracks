...
- **GPX is hand-written, and the parser is forgiving on purpose.** `gpx.ts` serialises and parses GPX 1.1 without a dependency — nine element types, small enough that a library would be more surface than the code it replaces. A file may come from any plotter, so structure it does not recognise is ignored rather than rejected, but values are checked hard: a blank `lat=""` must be dropped before `Number()` turns it into `0` and lands a point at null island, attributes may be single- or double-quoted, and an out-of-range character reference such as `&#999999999;` is kept verbatim rather than passed to `String.fromCodePoint`, which throws and would abort the whole import. **The parser sorts points by time**: TimeZero paginates its export in blocks of 150 and the seams overlap, so a point can be milliseconds older than the one before it — `src/__fixtures__/timezero-seams.gpx` reproduces this, and the real vendor exports are used when present and skipped when not. Coordinates are written at seven decimal places (~11 mm, finer than any GPS fix), so a round-trip moves a position by at most the 5.6 mm half-ULP bound. One file can hold several `<trk>` elements, which is what makes exporting a multi-selection produce a single file; the Signal K context rides in `<extensions>` so an export can round-trip a vessel's identity, which TimeZero's own files cannot.
