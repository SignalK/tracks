...
- **Both single-track routes read through `readSegments`.** The JSON route and `track.gpx` share one pipeline, so the same query cannot be answered two ways: the history reconciliation, the windowless fallback that still consults a provider, and the rule that a 404 means _neither source knows this vessel_ are subtle enough that a second copy drifts. Export is a route rather than client code for the same reason — `toGpx` stays the single serialiser. **Honouring that 404 rule takes a second, wider provider question**: a windowed read cannot tell an unknown vessel from one whose history lies outside the window, so a track about to 404 first asks the provider whether it knows the vessel at all — over all of recorded time rather than the requested window, since a provider filters its answer by the range it is given and the vessel being rescued is precisely the one whose data sits outside it — any bound later than the provider's oldest row would reintroduce the same false 404, so the bound is the epoch. Best-effort like every other provider call: a provider that lacks the method, errors or hangs leaves the 404 standing. **A provider that cannot be read at all is a third answer**: when the windowed read fails, the store holds nothing, and the probe says the vessel _is_ known, there is no track to serve and no basis for claiming there is none — that is a `503`, not an empty `200` and not a `404`, because reporting an outage as an empty history is the one answer certainly wrong. The store still wins wherever it can, since a provider enriches a query rather than answering it, and v2 never escalates: it has no per-context error, so an outage there degrades to stored points rather than failing a multi-context query for every other vessel in it. That answer is **cached briefly and shared while in flight**, because the probe runs only for a vessel about to 404 — precisely the request an open route can be asked to repeat without limit, which would otherwise mean one wide provider query per request. The whole context list is what gets cached, not a per-vessel verdict, or enumeration would just fill the cache with distinct keys; the cost is that a vessel a provider has newly learned about can take the cache window to become visible.
