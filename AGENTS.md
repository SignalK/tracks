...
- **Thinning and simplification are different operations.** `thin()` decimates by _time_ — one point per N milliseconds, whether the boat was turning or holding a course. `simplify()` (Douglas-Peucker, `simplify.ts`) decimates by _shape_, keeping the points that carry the geometry. A route conversion needs the second: thinning 5,550 recorded points to 30 gives 30 evenly spaced positions that miss every turn, while simplifying to 30 gives the turns. In the v2 provider simplification runs **after** thinning, and `epsilon` implies `simplify` per the spec; `simplify` alone derives a tolerance from the extent of the track as thinning left it — one part in a thousand of its diagonal, necessarily measured before simplification rather than after, since the post-simplification extent is what the tolerance produces, because a fixed metre value cannot suit both an ocean crossing and a marina approach.
